...
<div class="filter-container">
	{#each filters as filter}
		<div>
			<details id='filter-{filter.id}' role='list' class='filter filter-{filter.id}-container'>
				<summary class='filter-{filter.id}' aria-haspopup='listbox'>{filter.name}</summary>
				<ul role='listbox'>
					{#each filter.list as opt}
						<li>
							<label>
								{#if filter.type == "checkbox"}
									<input type={filter.type} checked={opt.checked}/>{opt.text}
								{:else}
									<input type={filter.type} name="filter-payed-option"/>{opt.text}
								{/if}
							</label>
						</li>
					{/each}
...
.filter-time-container, .filter-days-container, .filter-salary-container {
